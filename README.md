...
- Payments (requires JWT):
  ...
  - `GET /api/payments/config-status` (quick Razorpay config check)
...
- Keep secrets in backend only: `server/.env`
  - `RAZORPAY_KEY_ID=...`
  - `RAZORPAY_KEY_SECRET=...`
- Restart backend after any `.env` update.
- Do not mix modes: both keys must be from the same mode (`rzp_test_...` or `rzp_live_...`).
- Test config quickly:
  - `GET http://localhost:4000/api/payments/config-status`
  - If `paymentsConfigured` is `false`, fix env vars and restart backend.
