...
CREATE TABLE IF NOT EXISTS users (
  ...
  detailedSummaryEmails TINYINT(1) NOT NULL DEFAULT 0,
...
-- -----------------------
-- sent_monthly_summaries (dedupe monthly digest emails)
-- -----------------------
CREATE TABLE IF NOT EXISTS sent_monthly_summaries (
  id INT UNSIGNED NOT NULL AUTO_INCREMENT,
  userId INT UNSIGNED NOT NULL,
  ...
  updatedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uq_sent_monthly_user_month (userId, monthKey),
  CONSTRAINT fk_sent_monthly_user
    FOREIGN KEY (userId) REFERENCES users (id)
    ON DELETE CASCADE
) ENGINE=InnoDB;
